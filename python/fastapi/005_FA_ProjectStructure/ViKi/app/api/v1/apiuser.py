...
from models.UserModel import *  # Import UserModel for input parameter and return type  
...
from services.user import user_service as us  # Import UserService for business logic
...
@router.post("/upsertusers", response_model=UsersResponse)  
def upsert_users(request: UsersRequest):
    response = UsersResponse()
    model = UserModel()
    model.items =  request.items
    model =  us.upsert_users(model=model)
    response = model.multiResponse
    return response


@router.post("/deleteusers", response_model=UsersResponse)  
def delete_users(request: UsersRequest):
    response = UsersResponse()
    model = UserModel()
    model.items =  request.items
    model =  us.delete_users(model=model)
    response = model.multiResponse
    return response


@router.post("/update",response_model=UserResponse)  
def update_user(item: UserItem):
    response = UserResponse()
    model = UserModel()
    model.item = item
    model =   us.update_user(model=model)
    response = model.response
    return response

@router.post("/upsert",response_model=UserResponse)  
def upsert_user(item: UserItem):
    response = UserResponse()
    model = UserModel()
    model.item = item
    model =   us.upsert_user(model=model)
    response = model.response
    return response
...
def upsert_user(item: UserItem):
    ...
    model = UserModel()
    model.item = item
    model =   us.delete_user(model=model)
    response = model.response
    return response
